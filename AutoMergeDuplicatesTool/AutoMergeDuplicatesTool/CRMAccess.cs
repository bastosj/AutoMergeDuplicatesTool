    ...
    public static class CRMAccess
    {
        ...
        internal static void OpenConnection()
        {
            try
            {
                ...
                if (_orgService == null) { throw new InvalidCastException("Could not connect to Dataverse. Please contact your Administrator"); }

            }
            catch (Exception ex)
            {
                MessageBox.Show(string.Format("Connection problems with Dataverse: {0}", ex.Message));
            }
        }
